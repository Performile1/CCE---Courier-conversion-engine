    ...
    <div className="fixed inset-0 bg-black/50 z-[110] flex items-center justify-center p-4 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white w-full max-w-md shadow-2xl border-t-4 border-red-600 flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 space-y-4 overflow-y-auto">
          <div className="bg-slate-50 p-4 border border-slate-200 space-y-3 rounded-sm shadow-inner">
            ...
            <div className="max-h-[300px] overflow-y-auto space-y-2">
              {filteredSettings.length === 0 ? (
                ...
                filteredSettings.map(s => (
                  <div key={s.sniPrefix} className="flex items-center justify-between p-2.5 bg-white border border-slate-200 rounded-sm hover:shadow-sm transition-shadow">
                    <div className="flex items-center gap-3">
                       <span className="bg-red-600 text-white px-2 py-0.5 text-[10px] font-black rounded-sm">SNI {s.sniPrefix}</span>
                       <span className="text-xs font-black text-slate-700">{s.percentage}%</span>
                    </div>
